...
public class Detail_pinjamController implements Initializable {
    ...
    private void handleAddTransaction() {
        ...
        String query = "INSERT INTO transaksi (id_akun, id_transaksi, STATUS, tanggal_peminjaman, tanggal_pengembalian) "
                + "VALUES (?, ?, ?, ?, NULL)";
        ...
        try (Connection connection = dbConnection.getDBConnection(); PreparedStatement stmt = connection.prepareStatement(query)) {
            ...
            stmt.setInt(2, generateTransactionId(connection)); // Metode untuk membuat ID transaksi unik
            stmt.setString(3, status);
            stmt.setObject(4, tanggalPeminjaman); // LocalDate otomatis dikonversi ke datetime
